{
  ...
  sharedLibDeps ? {
    ...
    uvwasi = pkgs.uvwasi.overrideAttrs {
      version = "0.0.22";

      src = pkgs.fetchFromGitHub {
        owner = "nodejs";
        repo = "uvwasi";
        rev = "refs/heads/v0.0.22-proposal";
        hash = "sha256-vrtL9HDzbbbx3QmRsifRzNTYilC4zgo9JmJoA9haUFQ=";
      };
      postPatch = ''
        substituteInPlace CMakeLists.txt \
          --replace-fail 'list(APPEND uvwasi_cflags -fvisibility=hidden --std=gnu89)' 'list(APPEND uvwasi_cflags --std=gnu89)'
      '';
      cmakeFlags = [
