...
ifeq ($(SKIP_SHARED_DEPS), 1)
	...
	$(RM) -r $(TARNAME)
	gzip -c -f -$(GZ_COMPRESSION) $(TARNAME).tar > $(TARNAME).tar.gz
ifeq ($(XZ), 1)
...
$(TARBALL)-headers: release-only
	...
	$(RM) -r $(TARNAME)
	gzip -c -f -$(GZ_COMPRESSION) $(TARNAME)-headers.tar > $(TARNAME)-headers.tar.gz
ifeq ($(XZ), 1)
...
ifeq ($(OSTYPE),darwin)
	...
	$(RM) -r $(BINARYNAME)
	gzip -c -f -$(GZ_COMPRESSION) $(BINARYNAME).tar > $(BINARYNAME).tar.gz
ifeq ($(XZ), 1)
