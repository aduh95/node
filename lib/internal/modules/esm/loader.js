...
const { emitExperimentalWarning, setOwnProperty } = require('internal/util');
const {
  ...
  async eval(
    ...
    const evalInstance = (url) => {
      ...
      setCallbackForWrap(module, {
        ...
        importModuleDynamically: async (specifier, { url }, importAssertions) => {
          try {
            return await this.import(specifier, url, importAssertions);
          } catch (cause) {
            if (cause?.name !== 'SyntaxError') throw cause;
            const err = new ERR_DYNAMIC_IMPORT_SYNTAX_ERROR(specifier, url);
