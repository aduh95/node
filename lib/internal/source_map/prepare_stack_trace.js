...
function getErrorSource(
  ...
  const splitter = /\r?\n/g;
  let previousIndex, result;
  for (let i = 0; i <= originalLine; i++) {
    previousIndex = splitter.lastIndex;
    result = RegExpPrototypeExec(splitter, source);
    if (result === null && i < originalLine) return exceptionLine;
  }
  const line = StringPrototypeSlice(source, previousIndex, result?.index);
