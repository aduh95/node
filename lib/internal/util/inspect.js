...
function strEscape(str) {
  ...
  if (str.length > 100) {
    str = SideEffectFreeRegExpPrototypeSymbolReplace(escapeReplace, str, escapeFn);
    return addQuotes(str, singleQuote);
  }
...
function getFunctionBase(value, constructor, tag) {
  ...
  if (StringPrototypeStartsWith(stringified, 'class') && StringPrototypeEndsWith(stringified, '}')) {
    ...
    if (bracketIndex !== -1 &&
        ...
        RegExpPrototypeExec(classRegExp, SideEffectFreeRegExpPrototypeSymbolReplace(stripCommentsRegExp, slice)) !== null)
    ) {
...
function formatArrayBuffer(ctx, value) {
  ...
  let str = StringPrototypeTrim(SideEffectFreeRegExpPrototypeSymbolReplace(
    /(.{2})/g,
...
function formatIterator(braces, ctx, value, recurseTimes) {
  ...
  if (isKeyValue) {
    ...
    braces[0] = SideEffectFreeRegExpPrototypeSymbolReplace(/ Iterator] {$/, braces[0], ' Entries] {');
...
function formatProperty(ctx, value, recurseTimes, key, type, desc,
  ...
  if (typeof key === 'symbol') {
    const tmp = SideEffectFreeRegExpPrototypeSymbolReplace(
...
function stripVTControlCharacters(str) {
  ...
  return SideEffectFreeRegExpPrototypeSymbolReplace(ansi, str, '');
}
