...
function evalScript(name, body, breakFirstLine, print, shouldLoadESM = false) {
  ...
  const { kVmBreakFirstLineSymbol, setOwnProperty } = require('internal/util');
  ...
  const runScript = () => {
    ...
    const result = module._compile(script, `${name}-wrapper`)(() =>
      require('vm').runInThisContext(body, {
        ...
        async importModuleDynamically(specifier, _, importAssertions) {
          const loader = asyncESM.esmLoader;
          try {
            return await loader.import(specifier, baseUrl, importAssertions);
          } catch (cause) {
            if (cause?.name !== 'SyntaxError') throw cause;
            const err = new ERR_DYNAMIC_IMPORT_SYNTAX_ERROR(specifier, baseUrl);
            setOwnProperty(err, 'cause', cause);
            throw err;
          }
        },
      }));
